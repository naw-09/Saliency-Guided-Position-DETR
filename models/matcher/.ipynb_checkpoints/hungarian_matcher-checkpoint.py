...
class HungarianMatcher(nn.Module):
    ...
    def calculate_cost(
        ...
        # -------------------------------------------------
        # Adaptive Area-Aware Hungarian Matching
        # -------------------------------------------------
        lambda_small = 0.2
        ...
        # Only emphasize small objects
        small_mask = gt_area < 0.02
        
        small_weight = torch.ones_like(gt_area)
        small_weight[small_mask] += 0.2
            
        cost_bbox = cost_bbox * small_weight.unsqueeze(0)
        cost_giou = cost_giou * small_weight.unsqueeze(0)
        # -------------------------------------------------

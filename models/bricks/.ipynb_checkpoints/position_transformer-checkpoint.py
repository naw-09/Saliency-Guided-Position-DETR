...
class PositionTransformer(TwostageTransformer):
    ...
    def forward(
        ...
        # HFS: hierarchical foreground scoring and token filtering.
        # When use_hfs=False, the HFS path is bypassed and all tokens are
        ...
        if self.use_hfs:
            ...
            for level_idx in range(spatial_shapes.shape[0] - 1, -1, -1):
                ...
                level_mask = mask_flatten[:, start_index:end_index]
                ...
                if level_idx != spatial_shapes.shape[0] - 1:
                    ...
                    level_memory = (
                        level_memory
                        + level_memory * upsample_score * self.alpha[level_idx]
                    )

                score = self.enc_mask_predictor(level_memory)
                valid_score = score.squeeze(-1).masked_fill(
                    level_mask, score.min()
                )
            ...
            sort_index = torch.sort(
                selected_score, dim=1, descending=True
            )[1]
            selected_inds = torch.cat(selected_inds[::-1], 1).gather(
                1, sort_index
            )

            num_inds = selected_inds.shape[1]
            cast_dtype = (
                num_inds.dtype if torchvision._is_tracing() else torch.int64
            )
            layer_filter_ratio = (
                num_inds * self.layer_filter_ratio
            ).to(cast_dtype)
            selected_inds = [
                selected_inds[:, :r] for r in layer_filter_ratio
            ]

            position_score = position_score[::-1]
            foreground_score = self.flatten_multi_level(
                position_score
            ).squeeze(-1)
            ...
            # ASR-only ablation: disable HFS scoring/filtering.
...
class PositionTransformerEncoder(nn.Module):
    ...
    def forward(
        ...
        if use_hfs:
            # Original Position-DETR path with HFS-selected tokens.
            for layer_id, layer in enumerate(self.layers):
                inds_for_query = foreground_inds[layer_id].unsqueeze(-1).expand(
                    -1, -1, self.embed_dim
                )
                ...
                reference_points = torch.gather(
                    ori_reference_points.view(b, n, -1),
                    1,
                ...
                for i in range(foreground_inds[layer_id].shape[0]):
                    foreground_inds_no_pad = foreground_inds[layer_id][i][
                        :focus_token_nums[i]
                    ]
                    ...
                    outputs.append(
                        output[i].scatter(
                            0,
                            foreground_inds_no_pad.unsqueeze(-1).repeat(
                                1, query.size(-1)
                            ),
                            query_no_pad,
                        )
            ...
            if multi_level_masks is not None:
                background_embedding = [
                    self.background_embedding(mask).flatten(2).transpose(1, 2)
                    for mask in multi_level_masks
                ]
            ...
            # ASR-only path: all tokens are processed at every encoder layer.
            # FGFE is retained, but without HFS scores its top-k ranking uses
            # encoder classification confidence only.
            full_foreground_weight = torch.ones(
                (b, n), device=query.device, dtype=query.dtype
            )
            for layer in self.layers:
                query = output
                query_pos = ori_pos
                reference_points = ori_reference_points
                score_tgt = self.enhance_mcsp(query)
                output = layer(
                    query,
                    query_pos,
...
class AdaptiveTokenSaliencyBlock(nn.Module):
    """
    Token-level adaptive saliency refinement.
    ...
    Placement:
        HFS -> FGFE -> ASR -> Deformable Attention
    """

    def __init__(self, embed_dim=256, reduction=4):
        super().__init__()

        hidden_dim = max(embed_dim // reduction, 32)
        ...
        # Start as an identity mapping for stable optimization.
        self.gamma = nn.Parameter(torch.zeros(1))

    def forward(self, x):
        saliency = self.score(self.norm(x))
        return x * (1.0 + self.gamma * saliency)
...
class PositionTransformerEncoderLayer(nn.Module):
    def __init__(
        ...
        # Token-level Adaptive Saliency Refinement
        # Placement: HFS -> FGFE -> ASR -> Deformable Attention
        self.token_saliency = AdaptiveTokenSaliencyBlock(
            embed_dim=embed_dim,
            reduction=4,
        )
    ...
    def forward(
        ...
        # HFS -> FGFE -> ASR -> Deformable Attention
        # ======================================================
        query = self.token_saliency(query)

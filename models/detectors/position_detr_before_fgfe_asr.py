...
from models.bricks.saliency import AdaptiveSaliencyBlock
...
class PositionDETR(DNDETRDetector):
    def __init__(
        ...
        # Adaptive Saliency V1 on all neck feature levels.
        self.saliency = nn.ModuleList(
            [
                AdaptiveSaliencyBlock(ch)
                for ch in self.neck.num_channels
            ]
        )
...
# ======================================================
        ...
        # Adaptive Saliency V1 on all feature levels.
        multi_level_feats = [
            self.saliency[i](feature)
            for i, feature in enumerate(multi_level_feats)
        ]
        ...
        if self.training:
            ...
            # compute HFS foreground-position loss only when HFS is enabled
            if self.use_hfs:
                feature_stride = [
                    (
                ...
                focus_loss = self.focus_criterion(
                    foreground_mask,
                    targets,
                    feature_stride,
                    images.image_sizes,

...
def inference():
    ...
    if args.save_predictions:
        torch.save(predictions, args.save_predictions)
...
def _visualize_batch_for_infer(
    ...
    **kwargs,  # Not useful
    ...
    save_image = image[:, :, ::-1].copy()
    
    if save_image.dtype != np.uint8:
        save_image = np.clip(save_image, 0, 255).astype(np.uint8)
    
    ok = cv2.imwrite(save_path, save_image)
    
    print(
        f"SAVE={save_path} | "
        f"SUCCESS={ok} | "
        f"shape={save_image.shape} | "
        f"dtype={save_image.dtype}"
    )
